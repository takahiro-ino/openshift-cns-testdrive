...
このセクションでは、現在のストレージクラスターにODF Workerノードを追加する方法について説明します。

=== Workerノードを追加する

現在のストレージクラスターにODF Workerノードを追加します。その後、次のサブセクションでODFクラスターを拡張して新しいノードにストレージをプロビジョニングする方法について説明します。
...
The ODF team has released its own must-gather image for the must-gather tool that runs storage specific commands.
