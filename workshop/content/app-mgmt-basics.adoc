...
Now, delete the pods that belog to the *Deployment* `mapit`に属するPodをひとつ削除します。
...
また、*Pod* の名前が変わっています。これは、OpenShiftが現在の状態(*Pod* が1)が希望の状態(*Pod* が2)と一致していないことを即座に検出し、別の *Pod* をスケジューリングして修正したためです。
...
これらの *RS* をそれぞれ `oc describe` すると、`-1` にはProbeがなく、`-2` と `-3` にはそれぞれ新しいProbeがあることがわかるでしょう。
