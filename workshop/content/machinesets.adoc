...
oc scale machineset ${CLUSTERNAME}-worker-us-east-2c -n openshift-machine-api --replicas=2
----
...
続ける前に、先程スケールアップした `MachineSet` を1にスケールダウンしてください。

[source,bash,role="execute"]
----
CLUSTERNAME=$(oc get  infrastructures.config.openshift.io cluster  -o jsonpath='{.status.infrastructureName}')
oc scale machineset ${CLUSTERNAME}-worker-us-east-2c -n openshift-machine-api --replicas=1
----
